#include "TrieformProverK5Helper.h"

Cache TrieformProverK5Helper::persistentCache = Cache("P");

unsigned int TrieformProverK5Helper::assumptionsSize = 0;
GlobalSolutionMemo TrieformProverK5Helper::globalMemo = GlobalSolutionMemo();
unordered_map<string, unsigned int> TrieformProverK5Helper::idMap =
    unordered_map<string, unsigned int>();

shared_ptr<Trieform>
TrieformFactory::makeTrieK5Helper(const shared_ptr<Formula> &formula,
                                  shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie =
      shared_ptr<Trieform>(new TrieformProverK5Helper());
...
shared_ptr<Trieform>
TrieformFactory::makeTrieK5Helper(const vector<int> &newModality,
                                  shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie =
      shared_ptr<Trieform>(new TrieformProverK5Helper());
...
TrieformProverK5Helper::TrieformProverK5Helper() {}
TrieformProverK5Helper::~TrieformProverK5Helper() {}

shared_ptr<Trieform>
TrieformProverK5Helper::create(const shared_ptr<Formula> &formula) {
  return TrieformFactory::makeTrieK5Helper(formula, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverK5Helper::create(const shared_ptr<Formula> &formula,
                               const vector<int> &newModality) {
  if (newModality.size() > 0 &&
    ...
    return TrieformFactory::makeTrieK5Helper(formula, newModality,
                                             shared_from_this());
  }
  return TrieformFactory::makeTrieK5(formula, newModality, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverK5Helper::create(const vector<int> &newModality) {
  if (newModality.size() > 0 &&
      newModality[newModality.size() - 1] == s5Modality) {
    return TrieformFactory::makeTrieK5Helper(newModality, shared_from_this());
  }
  return TrieformFactory::makeTrieK5(newModality, shared_from_this());
}

shared_ptr<Bitset>
TrieformProverK5Helper::convertAssumptionsToBitset(literal_set literals) {
...
void TrieformProverK5Helper::updateSolutionMemo(
...
bool TrieformProverK5Helper::isInHistory(vector<shared_ptr<Bitset>> history,
                                         shared_ptr<Bitset> bitset) {
...
void TrieformProverK5Helper::reflexiveHandleBoxClauses() {
  ...
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
      dynamic_cast<TrieformProverK5Helper *>(modalSubtrie.second.get())
          ->reflexiveHandleBoxClauses();
    }
  }
}

void TrieformProverK5Helper::reflexivepropagateLevels() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
...
void TrieformProverK5Helper::pruneTrie() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
...
void TrieformProverK5Helper::makePersistence() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality)
      dynamic_cast<TrieformProverK5Helper *>(modalSubtrie.second.get())
          ->makePersistence();
...
void TrieformProverK5Helper::propagateSymmetricBoxes() {
  for (auto modalitySubtrie : subtrieMap) {
    if (modalitySubtrie.first == s5Modality) {

      dynamic_cast<TrieformProverK5Helper *>(modalitySubtrie.second.get())
...
void TrieformProverK5Helper::propagateEuclideaness() {
...
void TrieformProverK5Helper::preprocess() {
...
void TrieformProverK5Helper::prepareSAT(name_set extra) {
...
Solution TrieformProverK5Helper::prove(vector<shared_ptr<Bitset>> history,
  ...
  for (auto modalityDiamonds : triggeredDiamonds) {
    ...
    while (!diamondPriority.empty()) {
      ...
      if (getLevel() == 1) {
        ...
        childSolution = dynamic_cast<TrieformProverK5Helper *>(
...
Solution
TrieformProverK5Helper::prove(literal_set assumptions = literal_set()) {
  return prove(vector<shared_ptr<Bitset>>(), assumptions);
}

void TrieformProverK5Helper::setModality(int mod) { s5Modality = mod; }