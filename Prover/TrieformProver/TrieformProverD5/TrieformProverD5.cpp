...
TrieformProverD5::create(const shared_ptr<Formula> &formula,
  ...
  if (newModality.size() > 0) {
    shared_ptr<Trieform> result = TrieformFactory::makeTrieD5Helper(
        formula, newModality, shared_from_this());
    dynamic_cast<TrieformProverD5Helper *>(result.get())
...
shared_ptr<Trieform> TrieformProverD5::create(const vector<int> &newModality) {
  if (newModality.size() > 0) {
    shared_ptr<Trieform> result =
        TrieformFactory::makeTrieD5Helper(newModality, shared_from_this());
    dynamic_cast<TrieformProverD5Helper *>(result.get())
        ->setModality(newModality[newModality.size() - 1]);
    return result;
  }
...
bool TrieformProverD5::isInHistory(vector<shared_ptr<Bitset>> history,
...
shared_ptr<Bitset>
TrieformProverD5::convertAssumptionsToBitset(literal_set literals) {
  shared_ptr<Bitset> bitset =
...
void TrieformProverD5::updateSolutionMemo(const shared_ptr<Bitset> &assumptions,
                                          Solution solution) {
...
void TrieformProverD5::propagateEuclideaness() {
...
void TrieformProverD5::prepareSAT(name_set extra) {
...
Solution TrieformProverD5::prove(literal_set assumptions) {
  // Check solution memo
