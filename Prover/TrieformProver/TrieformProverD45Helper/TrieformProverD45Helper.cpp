#include "TrieformProverD45Helper.h"

Cache TrieformProverD45Helper::persistentCache = Cache("P");

unsigned int TrieformProverD45Helper::assumptionsSize = 0;
GlobalSolutionMemo TrieformProverD45Helper::globalMemo = GlobalSolutionMemo();
unordered_map<string, unsigned int> TrieformProverD45Helper::idMap =
    unordered_map<string, unsigned int>();

shared_ptr<Trieform>
TrieformFactory::makeTrieD45Helper(const shared_ptr<Formula> &formula,
                                   shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie =
      shared_ptr<Trieform>(new TrieformProverD45Helper());
...
shared_ptr<Trieform>
TrieformFactory::makeTrieD45Helper(const vector<int> &newModality,
                                   shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie =
      shared_ptr<Trieform>(new TrieformProverD45Helper());
...
TrieformProverD45Helper::TrieformProverD45Helper() {}
TrieformProverD45Helper::~TrieformProverD45Helper() {}

shared_ptr<Trieform>
TrieformProverD45Helper::create(const shared_ptr<Formula> &formula) {
  return TrieformFactory::makeTrieD45Helper(formula, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverD45Helper::create(const shared_ptr<Formula> &formula,
                                const vector<int> &newModality) {
  if (newModality.size() > 0 &&
    ...
    return TrieformFactory::makeTrieD45Helper(formula, newModality,
                                              shared_from_this());
  }
  return TrieformFactory::makeTrieD45(formula, newModality, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverD45Helper::create(const vector<int> &newModality) {
  if (newModality.size() > 0 &&
      newModality[newModality.size() - 1] == s5Modality) {
    return TrieformFactory::makeTrieD45Helper(newModality, shared_from_this());
  }
  return TrieformFactory::makeTrieD45(newModality, shared_from_this());
}

shared_ptr<Bitset>
TrieformProverD45Helper::convertAssumptionsToBitset(literal_set literals) {
...
void TrieformProverD45Helper::updateSolutionMemo(
...
bool TrieformProverD45Helper::isInHistory(vector<shared_ptr<Bitset>> history,
                                          shared_ptr<Bitset> bitset) {
...
void TrieformProverD45Helper::reflexiveHandleBoxClauses() {
  ...
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
      dynamic_cast<TrieformProverD45Helper *>(modalSubtrie.second.get())
          ->reflexiveHandleBoxClauses();
    }
  }
}

void TrieformProverD45Helper::reflexivepropagateLevels() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
...
void TrieformProverD45Helper::pruneTrie() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
...
void TrieformProverD45Helper::makePersistence() {
  for (auto modalSubtrie : subtrieMap) {

    shared_ptr<TrieformProverD45Helper> t = shared_ptr<TrieformProverD45Helper>(
        dynamic_cast<TrieformProverD45Helper *>(modalSubtrie.second.get()));
    if (t) {

      t->makePersistence();
    } else {
      dynamic_cast<TrieformProverD45 *>(modalSubtrie.second.get())
          ->makePersistence();
    }
  }
...
void TrieformProverD45Helper::propagateSymmetricBoxes() {
  for (auto modalitySubtrie : subtrieMap) {
    if (modalitySubtrie.first == s5Modality) {

      dynamic_cast<TrieformProverD45Helper *>(modalitySubtrie.second.get())
...
void TrieformProverD45Helper::propagateEuclideaness() {
...
void TrieformProverD45Helper::preprocess() {
...
void TrieformProverD45Helper::prepareSAT(name_set extra) {
...
Solution TrieformProverD45Helper::prove(vector<shared_ptr<Bitset>> history,
  ...
  for (auto modalityDiamonds : triggeredDiamonds) {
    ...
    while (!diamondPriority.empty()) {
      ...
      if (getLevel() == 1) {
        ...
        childSolution = dynamic_cast<TrieformProverD45Helper *>(
...
Solution
TrieformProverD45Helper::prove(literal_set assumptions = literal_set()) {
  return prove(vector<shared_ptr<Bitset>>(), assumptions);
}

void TrieformProverD45Helper::setModality(int mod) { s5Modality = mod; }