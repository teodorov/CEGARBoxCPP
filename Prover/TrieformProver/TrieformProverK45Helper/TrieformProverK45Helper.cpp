#include "TrieformProverK45Helper.h"

Cache TrieformProverK45Helper::persistentCache = Cache("P");

unsigned int TrieformProverK45Helper::assumptionsSize = 0;
GlobalSolutionMemo TrieformProverK45Helper::globalMemo = GlobalSolutionMemo();
unordered_map<string, unsigned int> TrieformProverK45Helper::idMap =
    unordered_map<string, unsigned int>();

shared_ptr<Trieform>
TrieformFactory::makeTrieK45Helper(const shared_ptr<Formula> &formula,
                                   shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie =
      shared_ptr<Trieform>(new TrieformProverK45Helper());
...
shared_ptr<Trieform>
TrieformFactory::makeTrieK45Helper(const vector<int> &newModality,
                                   shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie =
      shared_ptr<Trieform>(new TrieformProverK45Helper());
...
TrieformProverK45Helper::TrieformProverK45Helper() {}
TrieformProverK45Helper::~TrieformProverK45Helper() {}

shared_ptr<Trieform>
TrieformProverK45Helper::create(const shared_ptr<Formula> &formula) {
  return TrieformFactory::makeTrieK45Helper(formula, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverK45Helper::create(const shared_ptr<Formula> &formula,
                                const vector<int> &newModality) {
  if (newModality.size() > 0 &&
    ...
    return TrieformFactory::makeTrieK45Helper(formula, newModality,
                                              shared_from_this());
  }
  return TrieformFactory::makeTrieK45(formula, newModality, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverK45Helper::create(const vector<int> &newModality) {
  if (newModality.size() > 0 &&
      newModality[newModality.size() - 1] == s5Modality) {
    return TrieformFactory::makeTrieK45Helper(newModality, shared_from_this());
  }
  return TrieformFactory::makeTrieK45(newModality, shared_from_this());
}

shared_ptr<Bitset>
TrieformProverK45Helper::convertAssumptionsToBitset(literal_set literals) {
...
void TrieformProverK45Helper::updateSolutionMemo(
...
bool TrieformProverK45Helper::isInHistory(vector<shared_ptr<Bitset>> history,
                                          shared_ptr<Bitset> bitset) {
...
void TrieformProverK45Helper::reflexiveHandleBoxClauses() {
  ...
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
      dynamic_cast<TrieformProverK45Helper *>(modalSubtrie.second.get())
          ->reflexiveHandleBoxClauses();
    }
  }
}

void TrieformProverK45Helper::reflexivepropagateLevels() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
...
void TrieformProverK45Helper::pruneTrie() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.first == s5Modality) {
...
void TrieformProverK45Helper::makePersistence() {
  for (auto modalSubtrie : subtrieMap) {

    shared_ptr<TrieformProverK45Helper> t = shared_ptr<TrieformProverK45Helper>(
        dynamic_cast<TrieformProverK45Helper *>(modalSubtrie.second.get()));
    if (t) {
...
void TrieformProverK45Helper::propagateSymmetricBoxes() {
  for (auto modalitySubtrie : subtrieMap) {
    if (modalitySubtrie.first == s5Modality) {

      dynamic_cast<TrieformProverK45Helper *>(modalitySubtrie.second.get())
...
void TrieformProverK45Helper::propagateEuclideaness() {
...
void TrieformProverK45Helper::preprocess() {
...
void TrieformProverK45Helper::prepareSAT(name_set extra) {
...
Solution TrieformProverK45Helper::prove(vector<shared_ptr<Bitset>> history,
  ...
  for (auto modalityDiamonds : triggeredDiamonds) {
    ...
    while (!diamondPriority.empty()) {
      ...
      if (getLevel() == 1) {
        ...
        childSolution = dynamic_cast<TrieformProverK45Helper *>(
...
Solution
TrieformProverK45Helper::prove(literal_set assumptions = literal_set()) {
  return prove(vector<shared_ptr<Bitset>>(), assumptions);
}

void TrieformProverK45Helper::setModality(int mod) { s5Modality = mod; }