...
def compare_times(haskell_file, cpp_file):
    ...
    try:
        haskell_result = subprocess.check_output(
            ["/home/users/u6956078/.cabal/bin/CEGARBox", "--reflexive", haskell_file], timeout=TIMEOUT).decode("utf-8").strip()
    ...
    try:
        haskell_optim_result = subprocess.check_output(
            ["/home/users/u6956078/.cabal/bin/CEGARBoxOptim", "--reflexive", haskell_file], timeout=TIMEOUT).decode("utf-8").strip()
    except subprocess.TimeoutExpired:
    ...
    try:
        cpp_result = subprocess.check_output(
            ["/home/users/u6956078/mqbf/main", "-f", cpp_file, "--reflexive"], timeout=TIMEOUT).decode("utf-8").strip()
    except subprocess.TimeoutExpired:
        cpp_result = "timeout"
    except Exception as e:
        cpp_result = "timeout"
...
if __name__ == "__main__":
    ...
    for group in cpp_times:
        ...
        plt.legend()
        plt.title(group + " Benchmarks (T)")
        plt.ylabel("Problems Solved")
        plt.savefig("T" + group + ".png")
    print("Total")
